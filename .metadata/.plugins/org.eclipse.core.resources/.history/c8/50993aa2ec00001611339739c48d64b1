...
import comp417Project.DungeonBug01;
import comp417Project.DungeonBug02;
import comp417Project.DungeonBug03;
...
public class HandlerDungeonGeneration implements IWorldGenerator {
	...
	public static void doCreateDungeons(World world) {
		...
		int bugYOffset = 30;
		int bugZOffset = 50; 
		Pose bug1Pose = Pose.fromXyz(playerSpawnPose.getX() - 40 + bugXOffset, playerSpawnPose.getY() + height + bugYOffset, playerSpawnPose.getZ() - 10 + bugZOffset);
		System.out.println("Setting bug0Pose: " + bug1Pose);
		
		Pose bug2Pose = Pose.fromXyz(playerSpawnPose.getX() - 20 + bugXOffset, playerSpawnPose.getY() + height + bugYOffset, playerSpawnPose.getZ() - 40 + bugZOffset);
		System.out.println("Setting bug0Pose: " + bug1Pose);
		
		Pose bug3Pose = Pose.fromXyz(playerSpawnPose.getX() - 60 + bugXOffset, playerSpawnPose.getY() + height + bugYOffset, playerSpawnPose.getZ() - 90 + bugZOffset);
		System.out.println("Setting bug0Pose: " + bug2Pose);
		...
		BurlapCraft.registerDungeon(new DungeonBug01(bug1Pose));
		BurlapCraft.registerDungeon(new DungeonBug02(bug2Pose));
		BurlapCraft.registerDungeon(new DungeonBug03(bug3Pose));
